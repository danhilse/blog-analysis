...
def process_blog_batch(start_index: int = 0, batch_size: int = 100):
    """
    Process a batch of blog articles from a specified starting index.

    Args:
        start_index (int): The starting index for processing (default: 0)
        batch_size (int): Number of articles to process in this batch (default: 100)
    """
    # Read the JSON file
    ...
    # Get the articles from the specified range
    articles = all_data['analyses'].get('blog', [])

    if not articles:
        print("No blog articles found in the input file")
        return
    ...
    if start_index >= len(articles):
        print(f"Start index {start_index} exceeds number of available articles ({len(articles)})")
        return

    # Create subset of articles for this batch
    batch_articles = articles[start_index:end_index]

    # Create data structure expected by existing processing code
    batch_data = {
        'analyses': {
            'blog': batch_articles
        }
    ...
    output_file = f'output/blog_audit_{start_index}.xlsx'
    ...
    try:
        df = process_content_data(batch_data, output_file)
        print(f"Successfully processed batch. Output saved to: {output_file}")
        return df
    except Exception as e:
...
if __name__ == "__main__":
    ...
    for batch_num in range(total_passes):
        current_start = start + (batch_num * batch_size)
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        print(f"\n[{current_time}] Starting batch {batch_num + 1}/{total_passes}")
        print(f"Processing articles {current_start} to {current_start + batch_size - 1}")

        # First attempt
        try:
            df = process_blog_batch(start_index=current_start, batch_size=batch_size)
            print(f"Successfully completed batch {batch_num + 1}")
        except Exception as e:
            ...
            try:
                ...
                df = process_blog_batch(start_index=current_start, batch_size=batch_size)
                print(f"Successfully completed batch {batch_num + 1} on retry")
            except Exception as e:
                print(f"Second attempt failed: {str(e)}")
                print("Breaking execution after second failure")
                break

        # Don't wait after the last batch
        if batch_num < total_passes - 1:
